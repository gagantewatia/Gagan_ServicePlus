...
public class ImplementationofSchemeforProvidingImplementsMachineryonSubsidytotheFarmerofScheduleCasteCategoryDateTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoScheduleCasteCategory().click();
				test.log(Status.INFO,"Clicked on i icon"); 
				Assert.assertEquals(PQ.infoservicename2().getText(),"Implementation of Scheme for Providing Implements/Machinery on Subsidy to the Farmer of Schedule Caste Category SB-89/Last Date","Not matched");
				PQ.desccheck();
	...
	test.assignCategory(AgricultureDepartmentHaryana);
	AgricultureDepartmentHaryana dp=new AgricultureDepartmentHaryana(Driver);
	dp.ScheduleCasteCategory().click();
	Thread.sleep(2000);
	...
	Assert.assertTrue(s.contains("Implementation of Scheme for Providing Implements/Machinery on Subsidy to the Farmer of Schedule Caste Category SB-89/Last Date 10/01/2023"));
