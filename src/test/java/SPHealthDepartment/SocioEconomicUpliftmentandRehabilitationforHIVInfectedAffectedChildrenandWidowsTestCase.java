...
import Pageobject.SPHealthDepartment.HealthDepartment;
import Resources.Base2;

public class SocioEconomicUpliftmentandRehabilitationforHIVInfectedAffectedChildrenandWidowsTestCase extends Base2
{
	...
	{
		test.assignCategory(HealthDepartment);
...
//go to department page	
...

	{
		test.assignCategory(HealthDepartment);
		 departmentnavigationpage SD=new departmentnavigationpage(Driver);
		 String Servicename="Health Department (GMSH) Chandigarh";
	 	 SD.servicenavigationclick(Servicename);
		 Assert.assertEquals(SD.HealthDepartment().getText(),"Health Department (GMSH) Chandigarh","Not matched");
	...
	//go to service and check heading
...

		{
				...
				HealthDepartment dp=new HealthDepartment(Driver); 
				dp.infoSocioEconomic().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Socio-Economic Upliftment and Rehabilitation for HIV Infected/Affected Children and Widows","Not matched");
				PQ.desccheck();
	...
	Assert.assertTrue(s.contains("Socio-Economic Upliftment and Rehabilitation for HIV Infected/Affected Children and Widows"));
