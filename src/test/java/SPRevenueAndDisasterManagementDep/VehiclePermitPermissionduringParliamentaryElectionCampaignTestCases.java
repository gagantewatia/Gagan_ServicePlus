...
public class VehiclePermitPermissionduringParliamentaryElectionCampaignTestCases extends Base2
{
...
//go to department page	
...

	{
			...
			dp.infoElectionCampaign().click();
			test.log(Status.INFO,"Clicked on i icon");
			Assert.assertEquals(PQ.infoservicename2().getText(),"Vehicle Permit/ Permission during Parliamentary Election Campaign","Not matched");
			PQ.desccheck();
...
@Test(priority=6,dependsOnMethods = {"DeptNavigation"})
	...
	Assert.assertTrue(s.contains("Vehicle Permit/ Permission during Parliamentary Election Campaign"));
