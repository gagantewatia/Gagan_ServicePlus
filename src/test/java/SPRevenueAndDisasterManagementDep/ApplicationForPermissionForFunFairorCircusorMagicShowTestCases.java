...
public class ApplicationForPermissionForFunFairorCircusorMagicShowTestCases extends Base2
{
...
//go to department page	
...

	{
		test.assignCategory(RevenueAndDisasterManagementDep);
		 departmentnavigationpage SD=new departmentnavigationpage(Driver);
		 String Servicename="Revenue and Disaster Management Department (Haryana)";
...

	{
			...
			dp.infoMagicShow().click();
			test.log(Status.INFO,"Clicked on i icon");
			Assert.assertEquals(PQ.infoservicename2().getText(),"Application For Permission For Fun Fair or Circus or Magic Show","Not matched");
			PQ.desccheck();
...
@Test(priority=6,dependsOnMethods = {"DeptNavigation"})
	...
	Assert.assertTrue(s.contains("Application For Permission For Fun Fair or Circus or Magic Show"));
