...
public class VehiclePermitPermissionduringAssemblyStateElectionCampaignTestCases extends Base2
{
...
//go to department page	
...

	{
			...
			dp.infoStateElectionCampaign().click();
			test.log(Status.INFO,"Clicked on i icon");
			Assert.assertEquals(PQ.infoservicename2().getText(),"Vehicle Permit/ Permission during Assembly / State Election Campaign","Not matched");
			PQ.desccheck();
...
@Test(priority=6,dependsOnMethods = {"DeptNavigation"})
	...
	test.assignCategory(RevenueAndDisasterManagementDep);
	RevenueAndDisasterManagementDep dp=new RevenueAndDisasterManagementDep(Driver);
	dp.StateElectionCampaign().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Vehicle Permit/ Permission during Assembly / State Election Campaign"));
