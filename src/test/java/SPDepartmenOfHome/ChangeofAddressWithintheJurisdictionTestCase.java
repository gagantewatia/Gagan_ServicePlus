...
public class ChangeofAddressWithintheJurisdictionTestCase extends Base2
{
...
//go to department page	
...

		{
				...
				Infopage PQ=new Infopage(Driver);
				 DepartmenOfHome dp=new DepartmenOfHome(Driver);
					dp.infoJurisdiction().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Change of Address Within the Jurisdiction","Not matched");
				PQ.desccheck();
	...
	Assert.assertTrue(s.contains("Change of Address Within the Jurisdiction"));
