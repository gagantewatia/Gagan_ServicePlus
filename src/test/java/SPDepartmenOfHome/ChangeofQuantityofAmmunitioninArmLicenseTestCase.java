...
public class ChangeofQuantityofAmmunitioninArmLicenseTestCase extends Base2
{
...
//go to department page	
...

		{
				...
				Infopage PQ=new Infopage(Driver);
				 DepartmenOfHome dp=new DepartmenOfHome(Driver);
					dp.infoAmmunitioninArm().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Change of Quantity of Ammunition in Arm License","Not matched");
				PQ.desccheck();
	...
	Assert.assertTrue(s.contains("Change of Quantity of Ammunition in Arm License"));
