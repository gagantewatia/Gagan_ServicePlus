...
public class CasteCertificateforScheduledCastebySDOandDCfortheareanotcoveredunderULBorRLBTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoScheduledCasteSDO().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Caste Certificate for Scheduled Caste by SDO and DC for the area not covered under ULB or RLB","Not matched");
				PQ.desccheck();
	...
	{
	test.assignCategory(DepOfPersonnelAdministrativeReforms);		DepOfPersonnelAdministrativeReforms dp=new DepOfPersonnelAdministrativeReforms(Driver);
	//click on the service
	dp.ScheduledCasteSDO().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Caste Certificate for Scheduled Caste by SDO and DC for the area not covered under ULB or RLB"));
