...
public class CasteCertificateforEBCIandBCIINonCreamyLayerfortheareanotcoveredunderULBorRLBTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoCasteCertificateNonCreamyLayer().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Caste Certificate for EBC I and BC II (Non Creamy Layer) for the area not covered under ULB or RLB","Not matched");
				PQ.desccheck();
	...
	dp.CasteCertificateNonCreamyLayer().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Caste Certificate for EBC I and BC II (Non Creamy Layer) for the area not covered under ULB or RLB"));
