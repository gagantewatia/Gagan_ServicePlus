...
public class LocalResidentCertificatebyCircleOfficerfortheareanotliesunderULBorRLBTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoResidentCertificate().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Local Resident Certificate by Circle Officer for the area not lies under ULB or RLB","Not matched");
				PQ.desccheck();
	...
	public void ServiceNavigationTestCase() throws IOException, InterruptedException		
		{
	test.assignCategory(DepOfPersonnelAdministrativeReforms);
	DepOfPersonnelAdministrativeReforms dp=new DepOfPersonnelAdministrativeReforms(Driver);		//click on the service
	dp.ResidentCertificate().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Local Resident Certificate by Circle Officer for the area not lies under ULB or RLB"));
