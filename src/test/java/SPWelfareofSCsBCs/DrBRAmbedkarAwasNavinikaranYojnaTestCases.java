...
public class DrBRAmbedkarAwasNavinikaranYojnaTestCases extends Base2
{
   //go to all services
	@Test(priority=1)
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoAmbedkarAwas().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Dr B.R Ambedkar Awas Navinikaran Yojna","Not matched");
				PQ.desccheck();
	...
	Assert.assertTrue(s.contains("Dr B.R Ambedkar Awas Navinikaran Yojna"));
