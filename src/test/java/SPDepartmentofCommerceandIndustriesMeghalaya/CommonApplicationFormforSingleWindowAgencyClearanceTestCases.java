package SPDepartmentofCommerceandIndustriesMeghalaya;
...
import Pageobject.SPDepartmentofCommerceandIndustriesMeghalaya.DepartmentofCommerceandIndustriesMeghalaya;
import Resources.Base2;

public class CommonApplicationFormforSingleWindowAgencyClearanceTestCases extends Base2
{
	...
	{
		test.assignCategory(DepartmentofCommerceandIndustriesMeghalaya);
...
//go to department page	
...

	{
		test.assignCategory(DepartmentofCommerceandIndustriesMeghalaya);
		 departmentnavigationpage SD=new departmentnavigationpage(Driver);
		 String Servicename="Department of Commerce and Industries Meghalaya";
	 	 SD.servicenavigationclick(Servicename);
		 Assert.assertEquals(SD.DepartmentofCommerceandIndustriesMeghalaya().getText(),"Department of Commerce and Industries Meghalaya","Not matched");
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				DepartmentofCommerceandIndustriesMeghalaya dp=new DepartmentofCommerceandIndustriesMeghalaya(Driver);
				dp.infoCommonApplicationForm().click();
				test.log(Status.INFO,"Clicked on i icon"); 
				Assert.assertEquals(PQ.infoservicename2().getText(),"Common Application Form (CAF) for Single Window Agency (SWA) Clearance","Not matched");
				PQ.desccheck();
	...
	dp.CommonApplicationForm().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Common Application Form (CAF) for Single Window Agency (SWA) Clearance"));
