...
public class OnlineSewerConnectionTestCase extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoOnlineSewerConnection().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Online Sewer Connection","Not matched");
				PQ.desccheck();
	...
	test.assignCategory(DepOfUrbanDevelopment);
	DepOfUrbanDevelopment dp=new DepOfUrbanDevelopment(Driver);
	dp.OnlineSewerConnection().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
     Driver.switchTo().window(browserTabs.get(1));
	...
	Assert.assertTrue(s.contains("Online Sewer Connection"));
