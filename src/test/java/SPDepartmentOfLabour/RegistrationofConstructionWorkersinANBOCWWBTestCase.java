...
public class RegistrationofConstructionWorkersinANBOCWWBTestCase extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				Thread.sleep(2000);
				dp.infoConstructionWorkers().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Registration of Construction Workers in AN BOCWWB","Not matched");
				PQ.desccheck();
	...
	test.assignCategory(DepartmentOfLabour);
	DepartmentOfLabour dp=new DepartmentOfLabour(Driver);
	String s = dp.PHConstructionWorkers().getText();
	Assert.assertTrue(s.contains("A & N BUILDING AND OTHER CONSTRUCTION WORKERS WELFARE BOARD"));
