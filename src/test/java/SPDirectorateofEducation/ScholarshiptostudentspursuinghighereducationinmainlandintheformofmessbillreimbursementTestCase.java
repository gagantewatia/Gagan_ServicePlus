...
public class ScholarshiptostudentspursuinghighereducationinmainlandintheformofmessbillreimbursementTestCase extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoScholarshiptostudents().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Scholarship to students pursuing higher education in mainland in the form of mess bill reimbursement","Not matched");
				PQ.desccheck();
	...
	test.assignCategory(DirectorateofEducation);
	DirectorateofEducation dp=new DirectorateofEducation(Driver);
	dp.Scholarshiptostudents().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Scholarship to students pursuing higher education in mainland in the form of mess bill reimbursement"));
