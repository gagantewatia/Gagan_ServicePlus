...
public class MaternityBenefitSchemeforregisteredWomenworkerofHBOCWWBoardTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoMaternityBenefitScheme().click();
				test.log(Status.INFO,"Clicked on i icon"); 
				Assert.assertEquals(PQ.infoservicename2().getText(),"Maternity Benefit Scheme for registered Women worker of HBOCWW Board","Not matched");
				PQ.desccheck();
	...
	dp.MaternityBenefitScheme().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	Driver.switchTo().window(browserTabs.get(1));
	...
	Assert.assertTrue(s.contains("Maternity Benefit Scheme for registered Women worker of HBOCWW Board"));
