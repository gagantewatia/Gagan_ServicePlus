...
public class FinancialAssistanceforEducationofchildrenofregisteredworkerofHBOCWWBoardTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoFinancialAssistanceEducation().click();
				test.log(Status.INFO,"Clicked on i icon"); 
				Assert.assertEquals(PQ.infoservicename2().getText(),"Financial Assistance for Education of children of registered worker of HBOCWW Board","Not matched");
				PQ.desccheck();
	...
	dp.FinancialAssistanceEducation().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	Driver.switchTo().window(browserTabs.get(1));
	...
	Assert.assertTrue(s.contains("Financial Assistance for Education of children of registered worker of HBOCWW Board"));
