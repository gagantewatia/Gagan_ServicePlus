...
public class FinancialAssistancetoregisteredconstructionworkerofHBOCWWBoardforpurchaseconstructionofHouseTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoFinancialAssistanceconstruction().click();
				test.log(Status.INFO,"Clicked on i icon"); 
				Assert.assertEquals(PQ.infoservicename2().getText(),"Financial Assistance to registered construction worker of HBOCWW Board for purchase / construction of House","Not matched");
				PQ.desccheck();
	...
	dp.FinancialAssistanceconstruction().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	Driver.switchTo().window(browserTabs.get(1));
	...
	Assert.assertTrue(s.contains("Financial Assistance to registered construction worker of HBOCWW Board for purchase / construction of House"));
