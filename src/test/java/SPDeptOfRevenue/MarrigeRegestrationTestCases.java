...
public class MarrigeRegestrationTestCases extends Base2
{
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				DeptOfRevenue dp=new DeptOfRevenue(Driver);
				dp.infoMarrigeCertificate().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Marriage Registration Certificate","Not matched");
				PQ.desccheck();
	...
	Assert.assertTrue(s.contains("Marriage Registration Certificate"));
