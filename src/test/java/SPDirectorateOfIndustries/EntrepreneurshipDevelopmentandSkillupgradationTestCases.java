package SPDirectorateOfIndustries;
...
public class EntrepreneurshipDevelopmentandSkillupgradationTestCases extends Base2
{
	...
	{
		test.assignCategory(DirectorateOfIndustries);
...
//go to department page	
...

	{
		test.assignCategory(DirectorateOfIndustries);
		 departmentnavigationpage SD=new departmentnavigationpage(Driver);
		 String Servicename="Directorate of Industries Andaman & Nicobar Island";
	 	 SD.servicenavigationclick(Servicename);
		 Assert.assertEquals(SD.DirectorateOfIndustries().getText(),"Directorate of Industries Andaman & Nicobar Island","Not matched");
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				DirectorateOfIndustries dp=new DirectorateOfIndustries(Driver); 
				dp.infoEntrepreneurshipDevelopment().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Entrepreneurship Development and Skill up-gradation","Not matched");
				PQ.desccheck();
	...
	test.assignCategory(DirectorateOfIndustries);
	DirectorateOfIndustries dp=new DirectorateOfIndustries(Driver);
	dp.EntrepreneurshipDevelopment().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Entrepreneurship Development and Skill up-gradation"));
