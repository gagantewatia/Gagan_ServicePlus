...
public class IssuanceofBusinessRegistrationCertificateTestCase extends Base2
{
	...
	//go to department page	
		...
		//go to service and check heading
...

			{
					...
					dp.infoBusinessRegistrationCertificate().click();
					test.log(Status.INFO,"Clicked on i icon");
					Assert.assertEquals(PQ.infoservicename2().getText(),"Issuance of Business Registration Certificate","Not matched");
					PQ.desccheck();
	...
	test.assignCategory(KarbiAnglong);
	KarbiAnglong dp=new KarbiAnglong(Driver);
	dp.BusinessRegistrationCertificate().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Issuance of Business Registration Certificate"));
