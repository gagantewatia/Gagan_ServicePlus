...
public class AdditionQualificationandotherrelatedcertificatesTestCases extends Base2
{
   //go to all services
	@Test(priority=1)
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoAdditionQualification().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Addition Qualification and other related certificates","Not matched");
				PQ.desccheck();
	...
	test.assignCategory(EmploymentDepartment);
	EmploymentDepartment dp=new EmploymentDepartment(Driver);
	dp.AdditionQualification().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
	...
	Assert.assertTrue(s.contains("Addition Qualification and other related certificates"));
