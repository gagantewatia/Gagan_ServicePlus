...
public class RegistrationforSAKSHAMYUVASchemeTestCases extends Base2
{
   //go to all services
	@Test(priority=1)
...
//go to department page	
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoSAKSHAMYUVAScheme().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Registration for SAKSHAM YUVA Scheme","Not matched");
				PQ.desccheck();
	...
	Assert.assertTrue(s.contains("Registration for SAKSHAM YUVA Scheme"));
