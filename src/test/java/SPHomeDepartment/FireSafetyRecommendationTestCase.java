...
public class FireSafetyRecommendationTestCase extends Base2
{
...
//go to department page	
...

	{
			...
			dp.infoFireSafetyRecommendation().click();
			test.log(Status.INFO,"Clicked on i icon");
			Assert.assertEquals(PQ.infoservicename2().getText(),"Fire Safety Recommendation","Not matched");
			PQ.desccheck();
...
@Test(priority=6,dependsOnMethods = {"DeptNavigation"})
	...
	test.assignCategory(HomeDepartment);
	HomeDepartment dp=new HomeDepartment(Driver);
	dp.FireSafetyRecommendation().click();
	Thread.sleep(2000);
	List<String> browserTabs = new ArrayList<String> (Driver.getWindowHandles());
	//switch to new tab
     Driver.switchTo().window(browserTabs.get(1));
	...
	Assert.assertTrue(s.contains("Fire Safety Recommendation"));
