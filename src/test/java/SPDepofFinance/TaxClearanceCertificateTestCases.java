...
import Pageobject.SPDepOfFinance.DepatOfFinance;
import Resources.Base2;


public class TaxClearanceCertificateTestCases  extends Base2
{
			...
			{
				test.assignCategory(DepOfFinance);
...

			{
				test.assignCategory(DepOfFinance);
				 departmentnavigationpage SD=new departmentnavigationpage(Driver);
				 String Servicename="Department Of Finance Tripura";
			 	 SD.servicenavigationclick(Servicename);
				 Assert.assertEquals(SD.Deptoffinance().getText(),"Department Of Finance Tripura","Not matched");
			...
			public void Seeinfoclick() throws IOException, InterruptedException

				{
						...
						DepatOfFinance dp=new DepatOfFinance(Driver);;
						dp.infoTaxclearence().click();
						test.log(Status.INFO,"Clicked on i icon");
						Assert.assertEquals(PQ.infoservicename2().getText(),"Tax Clearance Certificate","Not matched");
						PQ.desccheck();
...

			{
				...
				Assert.assertTrue(getURL.contains("edistrict"));
			...
			String getURL = Driver.getCurrentUrl();
			System.out.println(getURL);
			Assert.assertTrue(getURL.contains("edistrict.tripura.gov"));
			}
			
			@Test(priority=8,dependsOnMethods = {"ServiceNavigationTestCase"})
			...
			Assert.assertTrue(s.contains("Tax Clearance Certificate"));
