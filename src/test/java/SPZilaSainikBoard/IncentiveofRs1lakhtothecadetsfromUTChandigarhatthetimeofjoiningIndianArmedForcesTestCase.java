...
import Pageobject.SPZilaSainikBoard.ZilaSainikBoard;
import Resources.Base2;

public class IncentiveofRs1lakhtothecadetsfromUTChandigarhatthetimeofjoiningIndianArmedForcesTestCase extends Base2
{
	...
	{
		test.assignCategory(ZilaSainikBoard);
...
//go to department page	
...

	{
		test.assignCategory(ZilaSainikBoard);
		 departmentnavigationpage SD=new departmentnavigationpage(Driver);
		 String Servicename="Zila Sainik Board Chandigarh";
	 	 SD.servicenavigationclick(Servicename);
		 Assert.assertEquals(SD.ZilaSainikBoard().getText(),"Zila Sainik Board Chandigarh","Not matched");
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				ZilaSainikBoard dp=new ZilaSainikBoard(Driver); 
				dp.infoIndianArmedForces().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Incentive of Rs 1 lakh to the cadets from UT Chandigarh at the time of joining Indian Armed Forces","Not matched");
				PQ.desccheck();
	...
	@Test(priority = 5)
		...
		{
			test.assignCategory(FoodSuppliesDepartmentHaryana);
	...
	Assert.assertTrue(s.contains("Incentive of Rs 1 lakh to the cadets from UT Chandigarh at the time of joining Indian Armed Forces"));
