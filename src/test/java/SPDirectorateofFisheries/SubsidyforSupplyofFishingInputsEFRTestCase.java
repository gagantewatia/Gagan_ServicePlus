...
public class SubsidyforSupplyofFishingInputsEFRTestCase extends Base2
{
	...
	public void Seeinfoclick() throws IOException, InterruptedException

		{
				...
				dp.infoFishingInputs().click();
				test.log(Status.INFO,"Clicked on i icon");
				Assert.assertEquals(PQ.infoservicename2().getText(),"Apply For The Office/Sub-Station Location","Not matched");
				PQ.desccheck();
	...
	String s = dp.HeadingFishingInputs().getText();
	Assert.assertTrue(s.contains("Apply For The Office/Sub-Station Location"));
